...
setRow.append(lbl,repsGroup,repsQuick,kgGroup,kgQuick,warmLbl,dupBtn,delBtn);
...
function toggleCompletionForDate(dateIso,opts={}){let preserveAnchor=!!opts.preserveAnchor;let reopenModal=!!opts.reopenModal;let dayName=nameFor(dateIso);if(s.cd&&s.cd[dateIso]){delete s.cd[dateIso];}else{s.cd[dateIso]=buildCompletedDayEntry(dateIso,dayName);}recomputePRFlagsFrom(dateIso);if(!preserveAnchor)s.last=latestCompletedDate();save();rToday();rCal();if(reopenModal)openModal(dateIso);}
