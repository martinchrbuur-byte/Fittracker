...
async function initApp(opts={}){try{if(!opts.skipLoad)load();setupTabs();setupModals();setupDevDebugPanel();wireStaticHandlers();rToday();rPlan();rCal(new Date());renderDevDebugPanel();}catch(err){console.error('[initApp] Initialization failed:',err);}}
