...
let statsUiState={period:'month',exerciseKey:''};
...
function statsGetWindow(period){let p=['week','month','year'].includes(period)?period:'month';let days=p==='week'?7:(p==='year'?365:30);let end=new Date();let start=new Date(end);start.setDate(start.getDate()-(days-1));return{period:p,days,startIso:iso(start),endIso:iso(end)};}
function statsFilterCompleted(period){let w=statsGetWindow(period);return Object.entries(s.cd||{}).filter(([dateIso,entry])=>entry&&typeof entry==='object'&&dateIso>=w.startIso&&dateIso<=w.endIso).sort((a,b)=>a[0].localeCompare(b[0])).map(([dateIso,entry])=>({dateIso,entry}));}
function statsBucketKey(dateIso,period){if(period==='week')return dateIso;if(period==='month')return getIsoWeekKey(dateIso);let d=fromIso(dateIso);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;}
function statsBucketLabel(key,period){if(period==='week'){return fromIso(key).toLocaleDateString('da-DK',{weekday:'short'});}if(period==='month'){let p=String(key||'').split('-W');return p.length===2?`U${p[1]}`:key;}let [yy,mm]=String(key||'').split('-');let m=parseInt(mm,10);if(!yy||!m)return key;return new Date(parseInt(yy,10),m-1,1).toLocaleDateString('da-DK',{month:'short'});}
function statsBuildBucketKeys(period){let w=statsGetWindow(period);if(period==='week'){let out=[];let d=fromIso(w.startIso);for(let i=0;i<w.days;i++){out.push(iso(d));d.setDate(d.getDate()+1);}return out;}if(period==='month'){let out=[];let seen=new Set();let d=fromIso(w.startIso);for(let i=0;i<w.days;i++){let k=getIsoWeekKey(iso(d));if(!seen.has(k)){seen.add(k);out.push(k);}d.setDate(d.getDate()+1);}return out;}let out=[];let seen=new Set();let d=fromIso(w.startIso);for(let i=0;i<w.days;i++){let k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;if(!seen.has(k)){seen.add(k);out.push(k);}d.setDate(d.getDate()+1);}return out;}
function statsFmtNumber(v){return (Math.round((toNum(v)||0)*100)/100).toLocaleString('da-DK');}
function statsCreateKpiCard(label,value){let c=C('div','stats-kpi-card');let l=C('div','card-subtitle');l.textContent=label;let v=C('div','stats-kpi-value');v.textContent=value;c.append(l,v);return c;}
function statsRenderLineChart(container,values,labels,yLabel){if(!container)return;let max=Math.max(0,...values);if(!values.length||max<=0){container.innerHTML='<div class="stats-empty">Ingen data i den valgte periode.</div>';return;}let width=640,height=190,padL=34,padR=14,padT=12,padB=34;let innerW=width-padL-padR,innerH=height-padT-padB;let points=values.map((v,i)=>{let x=padL+(innerW*(values.length===1?0:i/(values.length-1)));let y=padT+innerH-(innerH*((toNum(v)||0)/max));return{x,y,v};});let poly=points.map(p=>`${p.x},${p.y}`).join(' ');let yTicks=[0,Math.round(max/2),Math.round(max)];let tickText=yTicks.map((tv,idx)=>`<text x="4" y="${padT+innerH-(innerH*(idx/2))+4}" fill="var(--text-muted)" font-size="10">${tv}</text>`).join('');let circles=points.map(p=>`<circle cx="${p.x}" cy="${p.y}" r="3" fill="var(--accent)" />`).join('');let xLabels=points.map((p,i)=>{if(!(i===0||i===Math.floor((points.length-1)/2)||i===points.length-1))return '';return `<text x="${p.x}" y="${height-10}" text-anchor="middle" fill="var(--text-muted)" font-size="10">${labels[i]||''}</text>`;}).join('');container.innerHTML=`<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${yLabel}"><line x1="${padL}" y1="${padT+innerH}" x2="${width-padR}" y2="${padT+innerH}" stroke="var(--border-subtle)" /><line x1="${padL}" y1="${padT}" x2="${padL}" y2="${padT+innerH}" stroke="var(--border-subtle)" />${tickText}<polyline points="${poly}" fill="none" stroke="var(--accent)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />${circles}${xLabels}</svg>`;}
function statsRenderBarChart(container,values,labels,yLabel){if(!container)return;let max=Math.max(0,...values);if(!values.length||max<=0){container.innerHTML='<div class="stats-empty">Ingen data i den valgte periode.</div>';return;}let width=640,height=190,padL=28,padR=12,padT=12,padB=34;let innerW=width-padL-padR,innerH=height-padT-padB;let barW=Math.max(14,Math.floor(innerW/Math.max(values.length,1))-8);let gap=(innerW-(barW*values.length))/Math.max(values.length,1);let bars=values.map((v,i)=>{let x=padL+i*(barW+gap)+(gap/2);let h=innerH*((toNum(v)||0)/max);let y=padT+innerH-h;return`<rect x="${x}" y="${y}" width="${barW}" height="${Math.max(1,h)}" rx="4" fill="var(--accent)" />`;}).join('');let xLabels=values.map((_,i)=>{if(!(i===0||i===Math.floor((values.length-1)/2)||i===values.length-1))return '';let x=padL+i*(barW+gap)+(gap/2)+(barW/2);return `<text x="${x}" y="${height-10}" text-anchor="middle" fill="var(--text-muted)" font-size="10">${labels[i]||''}</text>`;}).join('');container.innerHTML=`<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${yLabel}"><line x1="${padL}" y1="${padT+innerH}" x2="${width-padR}" y2="${padT+innerH}" stroke="var(--border-subtle)" />${bars}${xLabels}</svg>`;}
function statsCollect(period){let rows=statsFilterCompleted(period);let bucketKeys=statsBuildBucketKeys(period);let volumeByBucket=Object.fromEntries(bucketKeys.map(k=>[k,0]));let sessionsByBucket=Object.fromEntries(bucketKeys.map(k=>[k,0]));let totalVolume=0,sessions=0,prCount=0;let exerciseMap={};let muscleMap={};let prEvents=[];rows.forEach(({dateIso,entry})=>{let bKey=statsBucketKey(dateIso,period);if(!(bKey in volumeByBucket)){volumeByBucket[bKey]=0;sessionsByBucket[bKey]=0;bucketKeys.push(bKey);}sessions++;sessionsByBucket[bKey]+=1;let logs=Array.isArray(entry.exerciseLogs)?entry.exerciseLogs:[];logs.forEach((log,idx)=>{let vol=toNum(log.volumeKg)||0;totalVolume+=vol;volumeByBucket[bKey]+=vol;let hasPr=!!(log.isPR_load||log.isPR_repsAtLoad||log.isPR_e1RM||log.isPR_volume);if(hasPr){prCount++;prEvents.push({dateIso,name:log.name||`Øvelse ${idx+1}`,kilos:(log.topSet&&log.topSet.kilos)||log.kilos||0,reps:(log.topSet&&log.topSet.reps)||log.reps||0});}let key=log.exerciseKey||log.id||`legacy:${log.name||idx}`;if(!exerciseMap[key])exerciseMap[key]={key,name:log.name||'Ukendt øvelse',volume:0,bestE1RM:0,sessions:0,points:[]};exerciseMap[key].volume+=vol;exerciseMap[key].bestE1RM=Math.max(exerciseMap[key].bestE1RM,toNum(log.estimated1RM)||0);exerciseMap[key].sessions+=1;exerciseMap[key].points.push({dateIso,e1RM:toNum(log.estimated1RM)||0,volume:vol});let muscles=Array.isArray(log.targetMuscles)&&log.targetMuscles.length?log.targetMuscles:(Array.isArray(log.bodyParts)&&log.bodyParts.length?log.bodyParts:['Ukendt']);muscles.forEach(m=>{let nm=String(m||'Ukendt').trim()||'Ukendt';muscleMap[nm]=(muscleMap[nm]||0)+vol;});});});let frequencyPerWeek=(sessions/(statsGetWindow(period).days/7));let exerciseList=Object.values(exerciseMap).sort((a,b)=>b.volume-a.volume);let muscleList=Object.entries(muscleMap).map(([name,volume])=>({name,volume})).sort((a,b)=>b.volume-a.volume).slice(0,8);let labels=bucketKeys.map(k=>statsBucketLabel(k,period));let volumeSeries=bucketKeys.map(k=>Math.round((volumeByBucket[k]||0)*100)/100);let frequencySeries=bucketKeys.map(k=>sessionsByBucket[k]||0);prEvents.sort((a,b)=>b.dateIso.localeCompare(a.dateIso));return{rows,labels,volumeSeries,frequencySeries,totalVolume:Math.round(totalVolume*100)/100,sessions,prCount,frequencyPerWeek,exerciseList,muscleList,prEvents:prEvents.slice(0,10)};}
function rStats(){let periodSel=$('stats-period-select'),exerciseSel=$('stats-exercise-select');let kpiWrap=$('stats-kpis'),volWrap=$('stats-volume-chart'),freqWrap=$('stats-frequency-chart'),progWrap=$('stats-progression-chart'),muscleWrap=$('stats-muscle-chart'),prList=$('stats-pr-list'),topList=$('stats-top-list');if(!kpiWrap||!volWrap||!freqWrap||!progWrap||!muscleWrap||!prList||!topList)return;let period=statsUiState.period||'month';if(periodSel&&periodSel.value!==period)periodSel.value=period;let data=statsCollect(period);kpiWrap.innerHTML='';kpiWrap.appendChild(statsCreateKpiCard('Total volumen',`${statsFmtNumber(data.totalVolume)} kg`));kpiWrap.appendChild(statsCreateKpiCard('Pas',`${data.sessions}`));kpiWrap.appendChild(statsCreateKpiCard('PR',`${data.prCount}`));kpiWrap.appendChild(statsCreateKpiCard('Pas/uge',`${statsFmtNumber(data.frequencyPerWeek)}`));statsRenderLineChart(volWrap,data.volumeSeries,data.labels,'Volumen');statsRenderBarChart(freqWrap,data.frequencySeries,data.labels,'Frekvens');let exOptions=data.exerciseList.slice(0,50);if(exerciseSel){let previous=statsUiState.exerciseKey||'';exerciseSel.innerHTML='';if(!exOptions.length){let o=C('option');o.value='';o.textContent='Ingen øvelse';exerciseSel.appendChild(o);statsUiState.exerciseKey='';}else{exOptions.forEach(ex=>{let o=C('option');o.value=ex.key;o.textContent=ex.name;exerciseSel.appendChild(o);});if(!exOptions.some(ex=>ex.key===previous))statsUiState.exerciseKey=exOptions[0].key;else statsUiState.exerciseKey=previous;exerciseSel.value=statsUiState.exerciseKey;}}let chosen=data.exerciseList.find(x=>x.key===statsUiState.exerciseKey)||null;if(!chosen){progWrap.innerHTML='<div class="stats-empty">Vælg en øvelse for at se progression.</div>';}else{let points=[...chosen.points].sort((a,b)=>a.dateIso.localeCompare(b.dateIso)).slice(-12);let vals=points.map(p=>Math.round((p.e1RM||0)*100)/100);let labels=points.map(p=>fromIso(p.dateIso).toLocaleDateString('da-DK',{day:'2-digit',month:'2-digit'}));statsRenderLineChart(progWrap,vals,labels,`Progression for ${chosen.name}`);}muscleWrap.innerHTML='';if(!data.muscleList.length){muscleWrap.innerHTML='<div class="stats-empty">Ingen muskeldata endnu.</div>';}else{let max=Math.max(...data.muscleList.map(x=>x.volume));data.muscleList.forEach(item=>{let row=C('div','stats-bar-row');let lb=C('span','stats-bar-label');lb.textContent=item.name;let track=C('div','stats-bar-track');let fill=C('div','stats-bar-fill');fill.style.width=`${max>0?Math.max(4,(item.volume/max)*100):0}%`;track.appendChild(fill);let val=C('span','stats-bar-value');val.textContent=`${statsFmtNumber(item.volume)} kg`;row.append(lb,track,val);muscleWrap.appendChild(row);});}prList.innerHTML='';if(!data.prEvents.length){prList.innerHTML='<div class="stats-empty">Ingen PR i perioden.</div>';}else{data.prEvents.forEach(ev=>{let row=C('div','stats-list-item');let l=C('span');l.textContent=`${ev.dateIso} · ${ev.name}`;let r=C('span','card-subtitle');r.textContent=`${ev.kilos}kg × ${ev.reps}`;row.append(l,r);prList.appendChild(row);});}topList.innerHTML='';if(!data.exerciseList.length){topList.innerHTML='<div class="stats-empty">Ingen øvelsesdata endnu.</div>';}else{data.exerciseList.slice(0,8).forEach(ex=>{let row=C('div','stats-list-item');let l=C('span');l.textContent=ex.name;let r=C('span','card-subtitle');r.textContent=`${statsFmtNumber(ex.volume)} kg`;row.append(l,r);topList.appendChild(row);});}}
...
function addExerciseForToday(dateIso,dayName,exerciseEntry){let list=ensureDateExerciseOverride(dateIso,dayName);list.push(ensureTodayOccurrence(exerciseEntry));let dayLog=getDayLog(dateIso,dayName);dayLog.plannedExercises=list;s.last=latestCompletedDate();let hadCompleted=!!(s.cd&&s.cd[dateIso]);if(hadCompleted)delete s.cd[dateIso];save();if(hadCompleted)rStats();}
function removeExerciseForToday(dateIso,dayName,index){let list=ensureDateExerciseOverride(dateIso,dayName);if(index<0||index>=list.length)return;let removed=list[index];list.splice(index,1);let dayLog=getDayLog(dateIso,dayName);dayLog.plannedExercises=list;let removedKey=occKeyForDateExercise(removed,index);if(dayLog.exercises&&dayLog.exercises[removedKey])delete dayLog.exercises[removedKey];s.last=latestCompletedDate();let hadCompleted=!!(s.cd&&s.cd[dateIso]);if(hadCompleted)delete s.cd[dateIso];save();if(hadCompleted)rStats();}
...
reps.addEventListener('input',()=>setReps(toNum(reps.value)||0));
reps.addEventListener('blur',()=>setReps(toNum(reps.value)||0));
kilos.addEventListener('input',()=>setKg(toNum(kilos.value)||0));
kilos.addEventListener('blur',()=>setKg(toNum(kilos.value)||0));
...
note.addEventListener('input',()=>{setExerciseProgress(t,nm,entryKey,entry,{exerciseNotes:note.value||''});});
...
function toggleCompletionForDate(dateIso,opts={}){let preserveAnchor=!!opts.preserveAnchor;let reopenModal=!!opts.reopenModal;flushProgressSave();let dayName=nameFor(dateIso);if(s.cd&&s.cd[dateIso]){delete s.cd[dateIso];}else{s.cd[dateIso]=buildCompletedDayEntry(dateIso,dayName);}recomputePRFlagsFrom(dateIso);if(!preserveAnchor)s.last=latestCompletedDate();save();rToday();rCal();rStats();if(reopenModal)openModal(dateIso);}
...
async function adminImportBackup(file){if(!adminCanAccess()){showSyncToast('Ingen admin-adgang','error');return;}if(!file||typeof syncImportState!=='function')return;let ok=await syncImportState(file);if(ok&&typeof syncFetchState==='function'){let state=await syncFetchState();if(state){load(state);rToday();rPlan();rCal(new Date());rStats();}}rAdmin();}
function setupTabs(){
	...
	let activateTab=(tab)=>{let tabEl=$("tab-"+tab);if(!tabEl)return;if(tab==='admin'&&!adminCanAccess())return;bs.forEach(x=>x.classList.remove("active"));cs.forEach(x=>x.classList.remove("active"));let targetBtn=[...bs].find(x=>x.dataset.tab===tab);if(targetBtn)targetBtn.classList.add("active");tabEl.classList.add("active");if(tab==='admin')rAdmin();if(tab==='statistik')rStats();};
...
function updateStickyOffsets(){let header=document.querySelector('.app-header');if(!header)return;let height=Math.ceil(header.getBoundingClientRect().height||0);if(height>0)document.documentElement.style.setProperty('--header-offset',`${height}px`);}
function setupTopbarAutoHide(){if(topbarScrollBound)return;topbarScrollBound=true;lastScrollY=Math.max(window.scrollY||window.pageYOffset||0,0);let onScroll=()=>{let y=Math.max(window.scrollY||window.pageYOffset||0,0);let delta=y-lastScrollY;if(y<24){document.body.classList.remove('topbar-hidden');lastScrollY=y;return;}if(delta>6&&y>80){document.body.classList.add('topbar-hidden');}else if(delta<-6){document.body.classList.remove('topbar-hidden');}lastScrollY=y;};window.addEventListener('scroll',onScroll,{passive:true});window.addEventListener('resize',()=>{document.body.classList.remove('topbar-hidden');lastScrollY=Math.max(window.scrollY||window.pageYOffset||0,0);});}
...
function wireStaticHandlers(){
	...
	bindOnce($("stats-period-select"),'StatsPeriod','change',()=>{let sel=$("stats-period-select");statsUiState.period=(sel&&sel.value)||'month';rStats();});
	bindOnce($("stats-exercise-select"),'StatsExercise','change',()=>{let sel=$("stats-exercise-select");statsUiState.exerciseKey=(sel&&sel.value)||'';rStats();});
