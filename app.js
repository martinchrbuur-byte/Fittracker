...
function load(stateObj){try{let f,r=localStorage;
 ...
 ensureStateShape();
 save();}catch(e){init();}}
function init(){s={split:D.slice(),w:{},n:{},last:null,ci:0,cd:{},wd:{},t:{...T0},a:{},pg:{},meta:{schemaVersion:SCHEMA_VERSION,statsLastComputedAt:null}};ensureStateShape();save();}
function save(){ensureStateShape();let r=localStorage;r.setItem("workouts",JSON.stringify(s.w));r.setItem("splitOrder",JSON.stringify(s.split));r.setItem("notes",JSON.stringify(s.n));r.setItem("lastPlannedDate",s.last||"");r.setItem("currentDayIndex",String(s.ci));r.setItem("completedDays",JSON.stringify(s.cd||{}));r.setItem("workoutDayLogs",JSON.stringify(s.wd||{}));r.setItem("templates",JSON.stringify(s.t||{}));r.setItem("appliedTemplates",JSON.stringify(s.a||{}));r.setItem("progressionGoals",JSON.stringify(s.pg||{}));r.setItem("stateMeta",JSON.stringify(s.meta||{}));let canSync=typeof syncStateDebounced==='function'&&typeof authIsLoggedIn==='function'&&authIsLoggedIn();if(canSync){syncStateDebounced(s);}}
...
setRow.append(lbl,repsGroup,kgGroup,warmLbl,dupBtn,delBtn);
...
function wireStaticHandlers(){
	...
	bindOnce($("admin-refresh"),'AdminRefresh','click',()=>{rAdmin();});
